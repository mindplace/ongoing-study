  ...
  def caesar(shift)
    ...
    letter_array.each_with_index do |letter, i|
      ...
      if new_letter > 122
        ...
        new_letter = new_letter.upcase if self[i] == self[i].upcase
...
class Fixnum
  ...
  def hash_for_stringify(num)
    num_set = {10 => "a", 11 => "b", 12 => "c", 13 => "d", 14 => "e", 15 => "f"}
    num_set[num]
  end
  ...
  def stringify(base)
    ...
    power = 1
    while true
      if i == 1 && base > 10 && num >= 10
        location = ((num / i) % base)
        item = hash_for_stringify(location)
      else
        item = ((num / i) % base).to_s
      end
      returning << item
      break if (num / (base ** power)) == 0
      i *= base
      power += 1
    end
    returning.reverse.join
  end
end
