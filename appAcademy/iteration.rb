...
class Array
  ...
  def bubble_sort(&prc)
    return prc.call if block_given?
...
def substrings(string)
  word = string.split("")
  combinations = []
  (1..string.length).to_a.each do |num| 
    combinations << word.combination(num).to_a.map{|set| set.join}
  end
  combinations.flatten.reverse
end

def subwords(word, dictionary)
  returning = []
  substrings(word).select do |string| 
    returning << string if dictionary.include?(string) && word.include?(string)
  end
  returning.uniq
end
...
def concatenate(strings)
  strings.inject{|result, bit| result + bit}
end
