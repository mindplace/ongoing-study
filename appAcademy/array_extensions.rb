...
class Array
  def my_uniq
    returning = []
    self.each do |x|
      returning << x if returning.include?(x) == false
    end
    returning
  end
end
...
class Array
  def two_sum
    returning = []
    self.each_with_index do |item1, indx1|
      self.each_with_index do |item2, indx2|
        next if indx1 >= indx2
        returning << [indx1, indx2] if item1 + item2 == 0
      end
    end
    returning
  end
...
class Array
  def median
    ...
    if array.length.odd?
      ...
      medians = array[((array.length / 2) - 1)..(array.length / 2)]
      medians.inject(:+).to_f / 2.to_f
    end
  end
end
